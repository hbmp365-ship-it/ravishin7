...
const EXTRA_IMAGE_VARIATIONS = [
  'Completely new scene: mid iron swing on the fairway, side view, full body. Wear a different color golf polo and cap than the reference photo.',
  'Completely new scene: walking on a cart path between holes on the course, candid smile, back three-quarter view, medium distance. Different windbreaker or vest outfit from the reference.',
  'Completely new scene: putting on an on-course green, low stance, side profile close shot. Different hat or visor and different top color from the reference.',
  'Completely new scene: standing near an on-course sand bunker looking at the hole, holding a club, front-facing casual pose, far full-body shot. Different golf skirt or pants and shoes from the reference.',
  'Completely new scene: resting on the fairway edge between shots, relaxed moment, three-quarter angle, medium shot. Different casual golf outfit from the reference (layered top or different polo).',
] as const;

export const buildAiProfileExtraImagePrompt = (slotIndex: number, ageNum?: string | null): string => {
  ...
  return [
    ...
    '- Do NOT reuse the same camera framing or crop',
